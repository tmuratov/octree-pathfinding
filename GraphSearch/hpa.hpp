	...
	if (cluster.size() <= 1)
		...
		for (auto a : cluster)
		{
			...
			{ // auto it = buffer.begin(); it != buffer.end(); it = buffer.begin() + i){
				for (auto tn : buffer[i]->neighbors)
				{
					search_node_ptr n = cast(tn);
					if (m_tree_nodes[n->id]->cluster_id != m_tree_nodes[buffer[i]->id]->cluster_id)
					{
						continue;
					}
					else if (m_tree_nodes[n->id])
					{
						// if this neighbor is already visited or added, refresh its shortest dist
						if (m_tree_nodes[n->id]->opened)
						{
							if (float x = Dijkstra_dist[buffer[i]] + dist(m_tree_nodes[n->id], buffer[i]); x < Dijkstra_dist[m_tree_nodes[n->id]])
							{
								Dijkstra_dist[m_tree_nodes[n->id]] = x;
								(m_tree_nodes[n->id])->prev_in_path = buffer[i];
							}
						}
						// else
						// {
...
void octree_path_planner::set_cluster_insides(std::vector<search_node_ptr> &cluster_inside, tree_node_ptr &curr)
{
	...
	// 		search_node_ptr a = convert_to_search_node(current);
...
void octree_path_planner::mark_treebranch(int c_id, search_node_ptr &c)
{
	// std::vector<search_node_ptr> queue;
	// queue.push_back(c);
	// for (int i = 0; i < queue.size(); i++)
	// {
	// 	if (auto x = queue[i]; x)
